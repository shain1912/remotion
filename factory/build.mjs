...
for (const sc of scenes) {
  ...
  // MiniMax retired — audio MUST be pre-generated by gen-voice.mjs (voicebox/MyVoice on H200).
  throw new Error(`audio not pre-generated for ${sc.id} — run: node factory/gen-voice.mjs ${projectId} --profile=MyVoice --force`);
}

// ---- 2) IMAGES (montage; pre-placed jpgs only — MiniMax retired; missing -> title-card fallback) ----
if (template === 'montage') {
  const have = scenes.filter((sc) => fs.existsSync(path.join(pub, 'img', `${sc.id}.jpg`))).length;
  console.log(`\n[image] montage — ${have}/${scenes.length} pre-placed jpgs (no generation)`);
} else {
...
const manifest = {
  ...
  scenes: scenes.map((sc) => {
    ...
    if (template === 'montage') {
      if (fs.existsSync(path.join(pub, 'img', `${sc.id}.jpg`))) base.image = `factory/${projectId}/img/${sc.id}.jpg`;
